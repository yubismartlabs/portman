...
type WorkspaceTab = "listeners" | "quarantine" | "posture";
  ...
  return <main className="desktop-shell">
    ...
    <section className="workspace">
      <div className="workspace-content">
        ...
        {workspaceTab === "quarantine" ? <QuarantinePanel entries={quarantine} watcher={watcher} onResume={async (pid) => { try { const result = await portmanApi.resumeQuarantined(pid); setNotice(result.message); setQuarantine((entries) => entries.map((entry) => entry.pid === pid && entry.state === "paused" ? { ...entry, state: "resumed", canResume: false } : entry)); } catch (cause) { setNotice(cause instanceof Error ? cause.message : "Could not resume the process."); } }} /> : workspaceTab === "posture" ? <PosturePanel posture={posture} onRefresh={() => portmanApi.posture().then(setPosture)} onFix={setFixing} /> : <section className={`main-panes ${selected ? "with-inspector" : ""}`}>
...
function Inspector({ listener, metrics, history, connections, anomaly, onClose, onStop }: { listener: Listener; metrics: ProcessMetrics | null; history: ProcessMetrics[]; connections: RemoteConnection[]; anomaly?: InstanceAnomaly; onClose: () => void; onStop: (listener: Listener) => void }) {
  const primary = listener.bindings[0]?.address ?? "127.0.0.1";
...
function Confirm({ listener, ports, force, busy, onClose, onConfirm }: { listener: Listener | null; ports: number[]; force: boolean; busy: boolean; onClose: () => void; onConfirm: () => void }) {
  ...
  useEffect(() => { if (!listener) { setRisk(null); return; } setRisk(null); portmanApi.stopRisk(listener.pid).then(setRisk).catch(() => setRisk(null)); }, [listener?.pid]);
