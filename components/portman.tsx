...
export function PortMan() {
  ...
  return <main className="desktop-shell">
    ...
    <aside className="app-sidebar">
      ...
      <div className="sidebar-section"><p>MEMORY GUARD</p><button className={memoryGuard?.enabled ? "sidebar-filter active" : "sidebar-filter"} disabled={!memoryGuard || controlUpdating === "memory"} onClick={() => void updateMemoryGuard(!memoryGuard?.enabled)}><i/>{memoryGuard?.enabled ? `Armed at ${memoryGuard.thresholdPercent}%` : "Enable protection"}</button></div>
      <div className="sidebar-section"><p>EXECUTION WATCHER</p><button className={watcher?.enabled ? "sidebar-filter active" : "sidebar-filter"} disabled={!watcher || controlUpdating === "watcher"} onClick={() => watcher && void updateExecutionWatcher(!watcher.enabled, watcher.autoPause)}><i/>{watcher?.enabled ? "Monitoring launches" : "Enable watcher"}</button><button className={watcher?.autoPause ? "sidebar-filter active" : "sidebar-filter"} disabled={!watcher?.enabled || controlUpdating === "watcher"} onClick={() => watcher && void updateExecutionWatcher(watcher.enabled, !watcher.autoPause)}><i/>{watcher?.autoPause ? "Auto-pause enabled" : "Review before pause"}</button></div>
